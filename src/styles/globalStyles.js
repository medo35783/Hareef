 ...
 * ───────────────────────
 * كل CSS المشترك في مكان واحد
 * يُحقن مرة واحدة في App.jsx
 *
 * الأقسام:
 *  1. Base & Variables
 *  2. Layout
 *  3. Components (btn, card, inp, badge...)
 *  4. Navigation
 *  5. Animations
 *  6. Game: Titles
 *  7. Game: Fameeri
...
.ptitle {
  font-family: 'Cairo', sans-serif;
  font-size: 23px; font-weight: 900; text-align: center; margin-bottom: 4px;
  background: linear-gradient(135deg, var(--gold), #ff8c00);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
}
...
.notif {
  ...
  z-index: 999; padding: 10px 18px; border-radius: 10px;
  font-weight: 700; font-size: 13px; text-align: center;
  animation: sdn .3s ease, fdo .3s ease 2.7s forwards;
...
.room-code-big {
  ...
  border: 2px dashed rgba(240,192,64,.3);
...
/* ── Grids ── */
.sg  { display: grid; gap: 6px; margin-bottom: 10px; }
...
.bnav {
  ...
  height: 62px; max-width: 720px; margin: 0 auto;
}
.bnav-item {
...
/* Nick tiles */
.nt { padding: 9px 5px; border: 1.5px solid rgba(240,192,64,.15); border-radius: 9px; text-align: center; cursor: pointer; transition: all .2s; font-size: 13px; font-weight: 700; color: var(--text); background: #09091e; min-height: 50px; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; }
...
.fc-success { border-color: var(--green); }
.fc-fail    { border-color: var(--red);   }
.fc-sys     { border-color: var(--gold);  }
...
@media (max-width: 390px) {
  .main  { padding: 10px; }
  .bgrid { grid-template-columns: repeat(2,1fr) !important; }
  .sg4   { grid-template-columns: 1fr 1fr !important; }
}
@media (min-width: 768px) {
  .main  { max-width: 640px; padding: 20px; }
  .card  { padding: 18px; }
